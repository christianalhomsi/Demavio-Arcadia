...
// Brand colors: primary blue, neon cyan, deep violet accent
const STAR_COLORS = ['0, 78, 235', '7, 177, 252', '99, 102, 241'];
...
function GravityStarsBackground({
  ...
  const updateStars = React.useCallback(() => {
    ...
    for (let i = 0; i < starsRef.current.length; i++) {
      ...
      if (dist < mouseInfluence && dist > 0) {
        ...
        if (mouseGravity === 'attract') { p.vx += nx * g; p.vy += ny * g; }
        else { p.vx -= nx * g; p.vy -= ny * g; }
  ...
  const drawStars = React.useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ...
      // Dark navy background matching --background: #01022A
      const bg = ctx.createLinearGradient(0, 0, ctx.canvas.width, ctx.canvas.height);
      bg.addColorStop(0, '#01022A');
      bg.addColorStop(0.5, '#01063B');
      bg.addColorStop(1, '#01022A');
      ctx.fillStyle = bg;
  ...
  React.useEffect(() => {
    if (starsRef.current.length === 0) {
      ...
      starsRef.current.forEach((p) => {
        ...
        if (spd > 0) { const ratio = movementSpeed / spd; p.vx *= ratio; p.vy *= ratio; }
      });
    }
