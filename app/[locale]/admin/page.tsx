        ...
        <div>
          ...
          <div className="rounded-2xl border border-border/50 bg-card overflow-hidden">
            {halls.slice(0, 5).map((h, i) => (
              <div key={h.id}
                className={`flex items-center justify-between px-5 py-3.5 hover:bg-muted/40 transition-colors ${i !== 0 ? "border-t border-border/30" : ""}`}>
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
                    style={{ background: "oklch(0.55 0.26 280 / 0.1)", border: "1px solid oklch(0.55 0.26 280 / 0.2)" }}>
                    <Building2 size={13} style={{ color: "oklch(0.65 0.22 280)" }} />
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{h.name}</p>
                    {h.address && <p className="text-xs text-muted-foreground truncate">{h.address}</p>}
                  </div>
                </div>
